...
| G-116 | 2026-09-03 | Expiry-day intraday basis capture: sell cash MIS + buy the discounted future inside expiry day, both flat by 15:20 (no borrow, no delivery) | cost-wall | eve variant: p75 of the negative eve discounts >= 0.75% AND >= 10% of eves <= -0.5%. Intraday variant, preregistered before the minute tape was opened: discount defined only where both legs traded in the minute, signal = two consecutive minutes <= -0.5% (prior minute <= -0.25%) after 09:20, entry at the next minute's open on both legs, exit at the 15:19 bar on both legs, net = gross - 0.25%; bars mean net > 0 with t >= 3 AND hit > 55% | EVE VARIANT DEAD AT THE GATE (12,426 symbol-expiries 2021-01..2026-06: eve discount median +0.03%, p90 of the negatives 0.30%, 1.2% of eves <= -0.5%, that tail dividend-payers). TWO DAILY-RESOLUTION ATTEMPTS UNINFORMATIVE, both artifacts caught in audit and kept on record: settle equals the cash close on expiry day so the daily P&L is circular (100% 'captured', t=42, a tautology), and Kite's continuous series rolls to the next contract on expiry day for ~13% of liquid name-days (dividend names: ITC, COALINDIA, PETRONET, INFY, WIPRO), so its 'cheap opens' were next-month dividend discounts; a corporate-action scale mismatch (adjusted cash bars vs raw futures, +906% 'best') caught the same night. THEN THE REAL TEST, on 1-minute bars of the expiring contracts, six 2025 expiries (Jan-Jun), 1,114 name-days, 253 excluded as the same corporate-action scale mismatch (absolute whole-day median discount above 2%, e.g. BAJFINANCE +898%, ASHOKLEY +100%), 861 clean. The tape passes its own sanity check: 15:19 discount median -0.005%. The expiring future trades within about 15bp of cash all day: day-median discount median -0.01%, p10 -0.17%, p90 +0.14%; hourly medians 09:30 -0.04%, 11:00 -0.01%, 14:00 -0.02%, 15:19 -0.005%, with p10 near -0.21% at every hour. The deepest single-minute discount on the median name-day is -0.33% (p10 -0.59%, p5 -0.71%); 16.7% of name-days touch -0.5% in some minute, 1.6% touch -1%. PRIMARY (-0.5% for two minutes): 19 of 861 name-days (2.2%), 85% of the signal discount converged by 15:19 (-0.57% to -0.08%), but the executable discount at the next minute's open was only about -0.24% (the two-minute print is the bottom tick; the next print is halfway back): gross +0.16% mean, +0.18% median; NET -0.09% mean, -0.07% median, t -1.35, hit 37%, worst -0.59%, best +0.39%. FAIL on every bar. SECONDARY (-0.25%): 425 name-days (49%), 239 of them signalling in the first hour, 79% converged, gross +0.14%, NET -0.11%, t -11.6, hit 28%. At -1.0%: zero events. The discount is real and it does converge; it is simply smaller than the round trip. Break-even all-in cost is about 0.14%, a market-maker's seat. Re-open only with a cost stack under 0.14% all-in, which is not a retail seat in India | 4 | expiry_eve_basis_prestudy.py, expiry_intraday_basis_test.py |
...
| G-114 | 2026-08-11 | A per-side 3x settle stop on the VRP sleeve improves the BOOK | handed-back | beat the book A/B on CAGR without worsening drawdown percentiles | Standalone the stop works, and plateau-stably across the stop multiples tried: better sleeve growth, a much shallower worst month, higher sleeve Sharpe. At book level REJECTED: a small gain in growth bought with worse drawdown percentiles and a lower solved leverage. The stop insures a solo-cell risk the book already diversifies away, then pays the premium in the joint chop-crash cell. Kept as a drawer item for standalone or high-weight VRP worlds | ~5 | SEAMS_AUDIT 2026-08-11, honest_update A/B |
...
---
*89 rows published in full or redacted form; 24 rows withheld pending leak review (counted, never hidden); 3 business and go-to-market kills counted but not shown, because this ledger is about dead trading strategies; 116 total. Generated from the internal master; regenerated on every change.*
